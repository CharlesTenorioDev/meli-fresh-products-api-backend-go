...
func (r *LocalityMysql) Save(locality *internal.Locality) (err error) {
	...
	result, err := r.db.Exec(
		"INSERT INTO `locality` (`name`, `province_name`, `country_name`) VALUES (?, ?, ?)",
		(*locality).LocalityName, (*locality).ProvinceName, (*locality).CountryName,
	)
	if err != nil {
...
func (r *LocalityMysql) ReportSellers(id int) (locality internal.Locality, err error) {
	// execute the query
	row := r.db.QueryRow("SELECT `l.id`, `l.name`, `l.province_name`, `l.country_name, COUNT(s.id)`, FROM `localities` AS `l` LEFT JOIN `sellers` as `s` ON `l.id` = `s.locality_id` WHERE `l.id` = ?", id)
...
func (r *LocalityMysql) FindByID(id int) (locality internal.Locality, err error) {
	// execute the query
	row := r.db.QueryRow("SELECT `l.id`, `l.name`, `l.province_name`, `l.country_name`, FROM `localities` AS `l` WHERE `l.id` = ?", id)
